...
# Function to load motor progress (Level & Tasks)
def load_motor_data():
    try:
        with open("player_data.json", "r") as file:
            ...
            # Fallback to the old format (nested under "motor")
                ...
                return 2, 0  # First-time default to Level 2, tasks start at 0
    except FileNotFoundError:
        return 2, 0  # First-time default to Level 2, tasks start at 0

# Load motor data only once at the start
current_level, motor_tasks = load_motor_data()

# Function to update motor level and task count dynamically
def save_motor_data(score):
    ...
    try:
        ...
        data = {}  # If file does not exist, create a new dictionary
    ...
    if score >= 60 and data["motor-level"] < 3:
        data["motor-level"] += 1  # Level Up
    elif score < 10 and data["motor-level"] > 1:
        data["motor-level"] -= 1  # Level Down
...
while running:
    ...
    if time_left <= 0 and game_state == "playing":
        ...
        # Save motor task progress (count task only once at the end of the game)
        save_motor_data(score)
