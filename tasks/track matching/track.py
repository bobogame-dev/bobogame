...
pygame.display.set_caption("Jungle Paw Matcher")
...
# Animal class
class Animal:
    def __init__(self, name, color, position):  # Fixed: Added `self`
        self.name = name
        self.color = color
...
class Paw:
    def __init__(self, color, position):
        ...
        # Load paw image
        self.image = pygame.image.load(f"assets/paw_{color}.png")
        self.image = pygame.transform.scale(self.image, (self.width, self.height))
    ...
    def draw(self, surface):
        ...
        if self.is_selected:
            outline_rect = pygame.Rect(self.rect.x - 5, self.rect.y - 5, self.rect.width + 10, self.rect.height + 10)
            pygame.draw.rect(surface, (255, 255, 0), outline_rect, 3, border_radius=15)
        if self.matched:
            outline_rect = pygame.Rect(self.rect.x - 5, self.rect.y - 5, self.rect.width + 10, self.rect.height + 10)
            pygame.draw.rect(surface, (0, 255, 0), outline_rect, 3, border_radius=15)
...
# HandCursor class
class HandCursor:
    def __init__(self):  # Fixed: Added `self`
        self.image = pygame.image.load("assets/hand.png")
        self.image = pygame.transform.scale(self.image, (100, 100))
        self.rect = self.image.get_rect()
        self.position = (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)
        self.is_grabbing = False
        self.selected_paw = None  # Track the currently selected paw
        self.previous_x = None  # Initialize previous_x
        self.previous_y = None  # Initialize previous_y
    ...
    def update(self, pos, is_grabbing, current_y):
        ...
        if is_grabbing:
            ...
            if self.is_grabbing:  # Just released grab
                ...
                if self.selected_paw is None:
                    self.is_grab_confirmed = False
                self.grab_timer = 0
            # If no paw is selected, make sure grab confirmed is always false when not grabbing
            if self.selected_paw is None:
                self.is_grab_confirmed = False
...
def setup_game():
    global animals, paws, animal_positions, paw_positions, matches_needed, game_background
    
    animals = []
    paws = []
    
    # Define animal types and corresponding paw colors
    animal_types = ["elephant2", "lion2", "monkey2", "giraffe2", "rabbit2", "bird2"]
    paw_colors = ["elephant", "lion", "monkey", "giraffe", "rabbit", "bird"]
    
    # Set matches_needed based on the current level
    update_game_parameters()
    
    # Define grid positions for paws (top of the screen)
    rows = 1  # One row for paws
    cols = matches_needed  # Number of columns based on matches needed
    paw_grid_width = SCREEN_WIDTH * 0.9  # Width of the paw grid
    paw_horizontal_spacing = paw_grid_width / cols
    paw_vertical_spacing = 100  # Fixed vertical spacing for paws
    
    paw_positions = []
    for i in range(cols):
        x = (SCREEN_WIDTH - paw_grid_width) // 2 + (i * paw_horizontal_spacing)
        y = 150  # Fixed y-position for paws at the top
        paw_positions.append((x, y))
    
    # Shuffle paw positions
    random.shuffle(paw_positions)
    
    # Define grid positions for animals (bottom of the screen)
    animal_grid_width = SCREEN_WIDTH * 0.9 # Width of the animal grid
    animal_horizontal_spacing = animal_grid_width / cols
    animal_vertical_spacing = 100  # Fixed vertical spacing for animals
    
    animal_positions = []
    for i in range(cols):
        x = (SCREEN_WIDTH - animal_grid_width) // 2 + (i * animal_horizontal_spacing)
        y = SCREEN_HEIGHT - 250  # Fixed y-position for animals at the bottom
    ...
    for i, animal_type in enumerate(animal_types[:matches_needed]):
        animal = Animal(animal_type, paw_colors[i], animal_positions[i])
        animals.append(animal)
    
    # Create paw objects
    for i, paw_color in enumerate(paw_colors[:matches_needed]):
        paw = Paw(paw_color, paw_positions[i])
        paws.append(paw)
    
    # Create initial game background with animals and paws
    game_background = create_game_background(animals, paws)

# Function to create the game background
def create_game_background(animals, paws):
    bg_surface = background.copy()
    
    # Draw paws on the background (green boxes)
    for paw in paws:
        if not paw.matched:
            bg_surface.blit(paw.image, paw.rect.topleft)
...
while running:
    ...
    if game_state == "playing":
        ...
        if show_tutorial:
            tutorial_text = [
                "Match the animals with their paws!",
                "1. Move your hand to the paw",
                "2. Hold your fist closed to grab it",
                "3. Move to the matching animal",
            ...
            # Draw the game background (with animals and paws)
            ...
            # Draw any selected paw or highlight effects that need to be on top of background
            if hand_cursor.selected_paw is not None:
                # Highlight selected paw
                glow_rect = pygame.Rect(
                    hand_cursor.selected_paw.rect.x - 15, 
                    hand_cursor.selected_paw.rect.y - 15, 
                    hand_cursor.selected_paw.rect.width + 30, 
                    hand_cursor.selected_paw.rect.height + 30
                )
                pygame.draw.rect(SCREEN, YELLOW, glow_rect, 3, border_radius=15)
            
            # Draw matched animals and paws with green glow
            for animal in animals:
                if animal.matched:
                    glow_rect = pygame.Rect(
                        animal.rect.x - 10, 
            ...
            # Handle paw selection and matching logic
            if hand_cursor.is_grab_confirmed:
                if hand_cursor.selected_paw is None:
                    # Try to select a paw
                    for paw in paws:
                        if not paw.matched and paw.rect.collidepoint(hand_cursor.position):
                            hand_cursor.selected_paw = paw
                            paw.is_selected = True
                            # Reset grab confirmation after selecting a paw
                            hand_cursor.is_grab_confirmed = False
                            hand_cursor.grab_timer = 0
                            break
                else:
                    # Try to match with an animal
                    for animal in animals:
                        if not animal.matched and animal.rect.collidepoint(hand_cursor.position):
                            if hand_cursor.selected_paw.color == animal.color:
                                # Correct match!
                                hand_cursor.selected_paw.matched = True
                                animal.matched = True
                                hand_cursor.selected_paw.is_selected = False
                                matches += 1
                    ...
                    # If not colliding with any animal, just keep the paw selected
    ...
    cv2.imshow("Jungle Paw Matcher", frame)
